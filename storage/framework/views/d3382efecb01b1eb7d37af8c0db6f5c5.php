...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <div class="w-full flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight"><?php echo e(__('Отзывы')); ?></h2>
            <a href="<?php echo e(route('reviews.create')); ?>" class="bg-gray-300 hover:bg-gray-600 hover:text-white text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center"><?php echo e(__('Новый отзыв')); ?></a>
        </div>
     <?php $__env->endSlot(); ?>
...
<?php endif; ?>
<?php $component->withAttributes([]); ?>
                <?php $__env->slot('spladeTableCell1cb251ec0d568de6a929b520c4aed8d1', function ($reviews) use ($__env) { ?>
                    <p class="max-w-[250px] overflow-x-auto"><?php echo e($reviews->text); ?></p>
                <?php }); ?>
                <?php $__env->slot('spladeTableCell78805a221a988e79ef3f42d7c5bfd418', function ($reviews) use ($__env) { ?>
                    <img class="max-w-[35px] rounded-full max-h-[35px] mt-[15px] mb-[15px]" src="<?php echo e(Storage::url($reviews->image)); ?>" alt="">
                <?php }); ?>
                <?php $__env->slot('spladeTableCell418c5509e2171d55b0aee5c2ea4442b5', function ($reviews) use ($__env) { ?>
                    <div class="flex gap-3">
                        <Link href="<?php echo e(route('reviews.destroy', $reviews->id)); ?>" class="bg-gray-300 hover:bg-gray-600 hover:text-white text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center" confirm="Внимание" confirm-text="Вы действительно хотите удалить отзыв?" confirm-button="Да" cancel-button="Нет" method="DELETE" ><?php echo e(__('Удалить')); ?></Link>
                        <Link href="<?php echo e(route('reviews.edit', $reviews->id)); ?>" class="bg-gray-300 hover:bg-gray-600 hover:text-white text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center" ><?php echo e(__('Редактировать')); ?></Link>
                        <Link href="<?php echo e(route('reviews.show', $reviews->id)); ?>" class="bg-gray-300 hover:bg-gray-600 hover:text-white text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center" ><?php echo e(__('Подробнее')); ?></Link>
                    </div>
                <?php }); ?>
                <?php $__env->slot('spladeTableCell367e854225a0810977297b3bedb2f309', function ($reviews) use ($__env) { ?>
                    <?php if($reviews->isActive == 0): ?>
                        <div class="bg-gray-300 text-black font-bold py-2 px-4 rounded inline-flex items-center">
                            <?php echo e(__('Не активна')); ?>

                        </div>
                    <?php elseif($reviews->isActive == 1): ?>
...
<?php /**PATH C:\OSPanel\domains\Ishak\resources\views/Admin/reviews/index.blade.php ENDPATH**/ ?>